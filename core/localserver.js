...
let E="data:application/javascript,export%20default%20",H=${JSON.stringify(__import__.origin)},d=(globalThis.__data__=e.data).cache,m={__proto__:null},R=(b,s,i=s.indexOf('^'),c=(i>=0?s=s.slice(0,i):s).charCodeAt(0))=>c==47||(c==46&&((c=s.charCodeAt(1))==47||(c==46&&s.charCodeAt(2)==47)))?new URL(s,b).href:c==c?s:b;(globalThis.__import__=(b,s='')=>import(R(b,s))).meta=u=>m[u]??=Object.freeze({url:u,resolve:s=>R(u,s)});(__import__.shim=${__import__.shim})?.();__import__.origin=H;for(let{0:k,1:v}of __import__.map=d){
