...
function prox(path){
	...
	fn = (...a) => {
		...
		for(const i of o) if(typeof i == 'string') str += i; else{
			const v = a[i-1]
			str += typeof v=='undefined'?'\\'+i:!v?'':typeof v.toLocaleString=='function'?v.toLocaleString():v
		}
		return str
	}
	fn.path = path
...
export const setLang = lang => fetch('langs/'+(lang || navigator.language.split('-',1)[0])+'.lang').then(a => a.status == 200 ? a.text() : fetch('langs/en.lang').then(a => a.text())).then(str => {
	langObj = new Map
	if(!str) return
