...
function helloPacket(data){
	...
	if(me._id > -1 && me._id != meid){
		if(e){
			addEntity(e)
		}else me = Entities.player({x:0,y:0,_id:-1,dx:0,dy:0,f:0})
	}else if(me.id == -1 && e)addEntity(e)
	r = data.byte()
...
function entityPacket(buf){
	while(buf.left){
		...
		if(!e)mv |= 256, e = EntityIDs[buf.short()]({x:0,y:0,_id: id,dx:0,dy:0,f:0,chunk:null})
		if(mv & 1)if(e == me || Math.abs(e.tx - (e.tx = buf.double())) > 16)e.x = e.tx
		if(mv & 2)if(e == me || Math.abs(e.ty - (e.ty = buf.double())) > 16)e.y = e.ty
