...
function iterate(list, dict, n, fngen){
	for(let i = 0; i < list.length; i++){
		...
		block._savedata = jsonToType(a.pop()||'null')
		block._savedatahistory = a.map(jsonToType)
		const f = fngen(block)
		f._ = block
		list[i] = f
		dict[name] = f
...
iterate(BlockIDs, Blocks, 'Blocks', block => block._savedata ? CONSTR.bind(undefined, block) : _ => block)
iterate(ItemIDs, Items, 'Item', item => ((obj, count, data = {}) => (data.count=count&255,Object.setPrototypeOf(data, obj))).bind(undefined, item))
