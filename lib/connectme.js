...
export function preconnect(ip){
	let displayIp = ip
	if(!/\w+:\/\//y.test(ip))ip = 'ws://' + ip
	if(!/:\d+$/.test(ip))ip += ':27277'
	const ws = new WebSocket(`${ip}/${storage.name}/${encodeURIComponent(storage.pubKey)}/${encodeURIComponent(storage.authSig)}`)
	ws.challenge = null
	...
	ws.onclose = () => {
		...
		icon.src = './img/no.png'
		motd.textContent = 'Failed to connect'
