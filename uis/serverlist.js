...
const serverList = UI('dirtbg serverlist',
	...
	Row(
		...
		skinCtx.canvas.attr('style','height:16rem;align-self:center;margin-left:3rem'),
		Label(storage.name).attr('style', 'color: #E92'),
		selectSkinBtn = Label(texts.serverlist.select_skin()).attr('style', 'color: #888; text-decoration: underline; cursor: pointer'),
		logoutBtn = Label(texts.serverlist.logout()).attr('style', 'color: #888; text-decoration: underline; cursor: pointer')
	),
	Spacer.grow(1),
	Row(
	...
	list = Div('serverlist'),
	Spacer.grow(1),
	Row(input = Input('text', texts.serverlist.add_server.placeholder()), addBtn = Btn(texts.serverlist.add_server(), () => {addServer(input.value);input.value='';addBtn.disabled=true}, 'small disabled')),
	Spacer.grow(1)
)
