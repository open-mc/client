...
precision mediump float;in vec2 uv;in vec4 tint;out vec4 color;uniform sampler2D tex1,tex2,tex3,tex4,tex5,tex6,tex7,tex8;uniform vec4 u1,u2;void main(){color=vec4(0,0,0,1);}`),g.compileShader(f);g.attachShader(p,v);g.attachShader(p,f);g.linkProgram(p);p.m=g.getUniformLocation(p,'g');p.a=g.getUniformLocation(p,'u1');p.b=g.getUniformLocation(p,'u2');p.t=[1,2,3,4,5,6,7,8].map(i=>g.getUniformLocation(p,'tex'+i));return p};N.useShader=(p=Q)=>(P=E,(E!=p)&&g.useProgram(E=p),P)}